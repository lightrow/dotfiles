...
class SquAeroSnap {
    ...
    SetHotkeyState(){
		fn := this.InitWindowForce.Bind(this)
		hotkey, #SC029, % fn
...
WIN + SHIFT + Left/Right = Resize left edge
	...
	TileWindow(win){
		...
		x := mon.TileCoords.x[win.Pos.x] -1
		y := mon.TileCoords.y[win.Pos.y] -1
		w := (mon.TileSizes.x * win.Span.x + 3)
		h := (mon.TileSizes.y * win.Span.y + 3)
...
Window_move(wndId, x, y, width, height) {
	static WM_ENTERSIZEMOVE = 0x231, WM_EXITSIZEMOVE  = 0x232
